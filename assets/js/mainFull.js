...
soundControl?.addEventListener('click', () => {
	if(musicPlayer.paused && !mobileCheck()) {
		musicPlayer.play(); 
		voicePlayer[activeSection - 1].volume = 1;
		voicePlayer[activeSection - 1].play();
		soundControl.classList.add('play');
		sound = true;
		...
		musicPlayer.pause();
		voicePlayer.forEach(player => player.pause());
		soundControl.classList.remove('play');
		sound = false;
	} else {
		musicPlayer.pause();
